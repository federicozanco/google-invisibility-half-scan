#custom
program_NAME := google-invisibility-half-scan
...
tar: $(program_BINARY)
	...
	@- cp $(program_C_SRCS) $(program_BINARY) Makefile Makefile.mingw README \
	      INSTALL CHANGELOG LICENSE COPYING $(program_FULLNAME)
	tar --remove-files --create --gzip --file $(program_TAR) $(program_FULLNAME)

src: 
	@- mkdir -p ./$(program_FULLNAME)
	@- cp $(program_C_SRCS) Makefile Makefile.mingw README INSTALL CHANGELOG \
	LICENSE COPYING $(program_FULLNAME)
	tar --remove-files --create --gzip --file $(program_SRC_TAR) $(program_FULLNAME)

# only for packaging
win: 
	@- mkdir -p ./$(program_FULLNAME)
	@- cp $(program_C_SRCS) $(program_WIN_BINARY) Makefile Makefile.mingw README \
	      INSTALL CHANGELOG LICENSE COPYING $(program_FULLNAME)
